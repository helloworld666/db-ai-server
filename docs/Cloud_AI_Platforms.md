...
> **v7.0更新**: 
> - 推荐使用 **LM Studio 本地部署** + **gemma-4-e4b-it** 模型
> - 通用模型（gemma）比代码专用模型（qwen2.5-coder）工具调用能力更强
> - 本地部署免费、隐私、完全可控
...
|------|----------|----------|-------------|
| `lmstudio` | LM Studio (本地) | **gemma-4-e4b-it** (推荐) | https://lmstudio.ai |
| `deepseek` | Deep Seek | deepseek-coder | https://platform.deepseek.com |
| `qwen` | 通义千问 (阿里云百炼) | qwen-plus, qwen2.5-7b-instruct | https://bailian.console.aliyun.com |
| `zhipu` | 智谱AI (GLM) | glm-4-9b-chat | https://open.bigmodel.cn |
...
**特点**：免费、隐私、完全本地、支持工具调用

**推荐模型**：
- `gemma-4-e4b-it` - **推荐**，工具调用能力强
- `qwen2.5-7b-instruct` - 通用模型
- `llama-3.1-8b-instruct` - Meta出品

**配置步骤**：
1. 下载安装 LM Studio：https://lmstudio.ai
2. 下载推荐模型（INT4量化，8G显存可用）
3. 启动 LM Studio，本地服务器（默认端口 62666）
4. 修改 `config/server_config.json`：
...
**模型选择建议**：
- 优先使用 **通用模型**（如 gemma、llama），而非代码专用模型
- 代码专用模型对工具调用指令遵循能力较弱

## 与本地模型对比

| 特性 | 云端AI平台 | LM Studio (本地) |
|------|-----------|----------------------------|
| 成本 | 按量付费/订阅 | 免费（本地运行） |
| 性能 | 强大（云端算力） | 依赖本地硬件（8G显存可用INT4量化） |
| 隐私 | 数据需上传 | 完全本地 |
| 网络 | 需要网络连接 | 离线可用 |
| 工具调用 | 通常良好 | 推荐使用通用模型 |
